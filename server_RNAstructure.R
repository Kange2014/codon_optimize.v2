...
folding_cal <- function(all_seqs = c()){
	all_seqs <- toupper(all_seqs)
	mfe_vec <- c()
	for(j in 1:length(all_seqs)){
		tmp <- gsub("T","U",all_seqs[j])
		a <- s2c(tmp)
		fasta <- list(a)
		write.fasta(fasta,"tmp01",file.out="~/tmpseq.fasta");
		
		system("Fold-smp ~/tmpseq.fasta ~/tmpseq.ct --loop 30 --maximum 20 --percent 10 --temperature 310.15 --window 3")
		
		raw_fold_data <- readLines("~/tmpseq.ct");
		mfes <- strsplit(raw_fold_data[1],"= "); ## minimum free energy
		mfes <- strsplit(mfes[[1]][2]," ");
		mfes <- mfes[[1]][1]
		mfe_vec <- c(mfe_vec,mfes)
		
		system("rm ~/tmpseq*");
	}
	
	deltaG <- data.frame(Seq=all_seqs,MFE=as.numeric(mfe_vec));
