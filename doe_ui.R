...
stat_tab_panel <- function(tool, tool_ui, output_panels,
                           data = input$dataset) {
  sidebarLayout(
    sidebarPanel(
      wellPanel(
        HTML(paste("<label><strong>", tool, "</strong></label><br>")),
        if (!is.null(data))
    ...
    wellPanel(
      tags$table(
        ...
        tags$td(numericInput("doe_trials", label = "# trials:", min = 0,
                   value = state_init("doe_trials", init = ""), width = "77px")), ## init = NA for windows version
